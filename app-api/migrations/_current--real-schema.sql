...
CREATE TABLE users (
    ...
    iam_provider VARCHAR(50), -- Identity provider (default, azure, cognito, etc.)
    ...
    azure_entra_id VARCHAR(255), -- Azure Entra ID
    azure_tenant_id VARCHAR(255), -- Azure tenant ID
    azure_object_id VARCHAR(255), -- Azure object ID
    azure_principal_name VARCHAR(255), -- Azure principal name
    ...
    azure_preferred_username VARCHAR(255), -- Azure preferred username
    ...
    auth_method VARCHAR(50), -- Authentication method (password, azure_entra, cognito, oidc)
...
CREATE TABLE ciam_systems (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(), -- Primary key
    system_name VARCHAR(50) NOT NULL UNIQUE, -- System identifier (cognito, azure_entra, etc.)
    display_name VARCHAR(100) NOT NULL, -- Human-readable name
    provider_type VARCHAR(50) DEFAULT 'oidc', -- Provider type (oidc, oauth2, saml, custom)
    is_active BOOLEAN DEFAULT TRUE, -- Active status
    
    -- Provider instance identification
    provider_instance_id VARCHAR(255) NOT NULL DEFAULT 'default', -- Instance identifier
    provider_environment VARCHAR(50) DEFAULT 'production', -- Environment
    provider_region VARCHAR(50), -- Provider region
    provider_domain VARCHAR(255), -- Provider domain
...
CREATE TABLE user_ciam_mappings (
    ...
    ciam_identifier VARCHAR(255) NOT NULL, -- CIAM user identifier
    ciam_system VARCHAR(50) NOT NULL, -- CIAM system name
    provider_identifier VARCHAR(255), -- Provider-specific identifier
    provider_metadata JSONB DEFAULT '{}', -- Provider-specific metadata
    ...
    authentication_method VARCHAR(50) DEFAULT 'oidc', -- Auth method
